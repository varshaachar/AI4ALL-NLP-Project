...
class Tweet(object):
  def __init__(self, tweetSurfaceForm, category, need_or_resource):
    self.tweetSurfaceForm = tweetSurfaceForm
  ...
  def __str__(self):
    return str(self.tweetSurfaceForm)
...
def read_csv(path):
   ...
   with open(path, encoding='utf8') as f:
     ...
     for row in reader:
       ...
       #data[tweetId] = [tweetText, category, need_or_resource]
...
def most_discriminative(tweets, token_probs, prior_probs):
  ...
  for token in all_tokens:
    ...
    log_dist = {c: l+min_log_dist for c,l in list(log_dist.items())} # shift so smallest value is 0 before taking exp
    dist = {c:math.exp(l) for c,l in list(log_dist.items())} # take exp
    s = sum(dist.values())
  ...
  for c in categories:
    probs = [(token,dist[c]) for token,dist in list(token2dist.items())]
...
def calc_probs(tweets, c):
    ...
    feature_probs = Counter({feature: float(count)/num_tweets_about_c for feature,count in list(feature_counts.items())})
